...
<div class="container mt-5">
  <div class="row">
  <h3> {{ __('translationFile.summarybyday') }}</h3><hr>
    <div class="col-md-6">
      <form method="GET" action="{{ route('reports.summarybyday') }}">
        <div class="mb-1">
  ...
  <table class="table">
    <thead>
      <tr>
        ...
        <th scope="col">{{ __('translationFile.day') }}</th>
    ...
    <tbody>
      ...
      <tr>
        <td>
          @if($language === 'en')  {{ \Carbon\Carbon::parse($total->eod_date)->format('D j M') }}  @else    {{ \Carbon\Carbon::parse($total->eod_date)->isoFormat('dddd D MMMM') }} @endif
         </td>
        <td>{{ __('translationFile.kd') }} {{ $total->total_amount }}</td>
        @php
    ...
    <tfoot>
        <tr>
          ...
          <td><b>{{ __('translationFile.kd') }}  {{ number_format($grand_total, 3) }}</b></td>
        </tr>
